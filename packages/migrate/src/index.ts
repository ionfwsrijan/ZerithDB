...
async function runMigration(
  ...
  console.log(
    chalk.bold.cyan("  ║    ZerithDB Migration Tool  v0.1.0   ║")
  );
  console.log(
    chalk.bold.cyan("  ╚══════════════════════════════════════╝")
  );
  ...
  try {
    ...
    console.log("");
    console.log(
      `  ${chalk.dim("Source:")}      ${chalk.white(result.adapter)}`
    );
    console.log(
      `  ${chalk.dim("Collections:")} ${chalk.white(result.totalCollections)}`
    );
    console.log(
      `  ${chalk.dim("Documents:")}   ${chalk.white(result.totalDocuments.toLocaleString())}`
    );
    console.log(
      `  ${chalk.dim("Duration:")}    ${chalk.white(formatDuration(result.durationMs))}`
    );
    console.log(
      `  ${chalk.dim("Output:")}      ${chalk.cyan(result.outputPath)}`
    );
    console.log("");
    ...
    if (warnings.length > 0) {
      ...
      if (warnings.length > 20) {
        console.log(
          chalk.dim(`    … and ${warnings.length - 20} more (check logs)`)
        );
      }
      console.log("");
    }
